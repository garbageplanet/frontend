...
gulp.task('styles:fonts', ['trimHTML'], function() {
  return gulp.src([               
...
gulp.task('styles:bootstrap', ['trimHTML'], function() {
  return gulp.src([
              './html/css/bootstrap-3.3.6.css',
              './html/css/libs/bootstrap-datetimepicker-4.17.37.css',
              './html/css/libs/bootstrap-select-1.9.4.css',
              './html/css/libs/bootstrap-tagsinput-0.4.3.css'
           ])
...
gulp.task('styles:leaflet', ['trimHTML'], function() {
  return gulp.src([
              './html/css/libs/leaflet-0.7.7.css',
              './html/css/libs/leafle.draw.css',
              './html/css/libs/L.Control.Sidebar-0.19a.css'
           ])
...
gulp.task('styles:app', ['trimHTML'], function() {
  return gulp.src([
              './html/css/main.css',
              './html/css/map.css',
              './html/css/marker.css,'
...
gulp.task('scripts:jquery', ['trimHTML'], function() {
  return gulp.src([
            ...
            './html/js/libs/jquery.ui.widget.js',
...
gulp.task('injectFiles', ['scripts:leaflet', 'scripts:jquery', 'scripts:app', 'styles:fonts', 'styles:bootstrap', 'styles:leaflet', 'styles:app'], function() {
  return gulp.src('./temp/index.html')
  // inject styles
    .pipe(inject(gulp.src('./dist/fonts.min.css', {read: false}), {starttag: '<!-- inject:head:css:fonts -->', ignorePath: 'dist', addRootSlash: false}))
    .pipe(inject(gulp.src('./dist/leaflet.min.css', {read: false}), {starttag: '<!-- inject:head:css:leaflet -->', ignorePath: 'dist', addRootSlash: false}))
    .pipe(inject(gulp.src('./dist/bootstrap.min.css', {read: false}), {starttag: '<!-- inject:head:css:bootstrap -->', ignorePath: 'dist', addRootSlash: false}))
    .pipe(inject(gulp.src('./dist/app.min.css', {read: false}), {starttag: '<!-- inject:head:css:app -->', ignorePath: 'dist', addRootSlash: false}))
  // inject scripts
    .pipe(inject(gulp.src('./dist/leaflet.min.js', {read: false}), {starttag: '<!-- inject:head:leaflet -->', ignorePath: 'dist', addRootSlash: false}))
    .pipe(inject(gulp.src('./dist/jquery.min.js', {read: false}), {starttag: '<!-- inject:head:jquery -->', ignorePath: 'dist', addRootSlash: false}))
...
gulp.task('default', ['clean:start'], function() {
    gulp.start('trimHTML', 'scripts:leaflet', 'scripts:jquery', 'scripts:app', 'styles:fonts', 'styles:bootstrap', 'styles:leaflet', 'styles:app', 'injectFiles', 'minifyHTML', 'clean:end', 'copy:fonts', 'copy:media');
});
