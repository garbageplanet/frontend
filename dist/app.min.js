...
var tools={setMarkerClassColor:function(c){return c===1?"marker-color-limegreen":c===2?"marker-color-yellow":c===3?"marker-color-orangered":c===4?"marker-color-red":"marker-color-violet"},setTodoFullText:function(t){return t==1?" it has been cleaned up already":t==2?" need help to clean it up":t==3?" full bags need to be collected":t==4?" a cleaning needs to be organized":" tell the local authorities"},setPolylineColor:function(c){return c===1?"#ccff66":c===2?"#ffff00":c===3?"#FF4500":c===4?"#ff1a1a":"#e60073"},getCurrentBounds:function(){var currentViewBounds=map.getBounds().toBBoxString();return currentViewBounds}};String.prototype.ellipsis=function(n){return this.substr(0,n-1)+(this.length>n?"&hellip;":"")};
var session=function(){var switchSession=function(sessionStatus){var classicSessionType=localStorage.getItem("classic");if(sessionStatus==="logout"){if(window.innerWidth<568){mobile.glomelogincontrol.logout()}$("#session-status a").text("Login").attr("href","#user-login-dialog");$("#session-status a").attr("id","");$("#session-status a").addClass("dropdown-link");$("#user-info-link").remove();$("#user-info-mobile-link").remove();$("#user-tools").dropdown();$(".user-email, .user-glome-key").removeClass("hidden");$(".session-link").removeClass("hidden")}if(sessionStatus==="login"){if(window.innerWidth<568){mobile.glomelogincontrol.login()}$("#session-status a").text("Logout").attr("href","#");$("#session-status a").attr("id","btn-logout");$("#session-status a").removeClass("dropdown-link");$("#session-status").on("click","#btn-logout",function(){switchSession("logout");logout()});$("#user-tools").prepend('<li id="user-info-link"><a class="dropdown-link" href="#account-info">User info</a></li>');$("#user-info-link a").on("click",function(e){e.preventDefault();$("#sidebar").scrollTop=0;$(this.hash).fadeIn().siblings().hide();sidebar.show()});$(".mobile-menu").append('<a href="#account-info" id="user-info-mobile-link" class="sidebar-link btn btn-default btn-lg btn-block"><span class="fa fa-fw fa-user"></span> User info</a>');$("#user-info-mobile-link").on("click",function(e){e.preventDefault();$("#sidebar").scrollTop=0;$(this.hash).fadeIn().siblings().hide()});$("#user-tools").dropdown();$(".session-link").addClass("hidden");if(classicSessionType==="true"){$("#account-info").find(".user-email").removeClass("hidden");$("#account-info").find(".user-name").text(localStorage.getItem("username"));$("#account-info").find(".user-email p").html(localStorage.getItem("useremail"));$("#account-info").find(".user-glome-key").addClass("hidden");$("#account-info").find(".user-id").html(localStorage.getItem("userid"));$(".sidebar-content").hide();if(!sidebar.isVisible()){sidebar.show()}$("#account-info").show()}if(classicSessionType==="false"){$("#account-info").find(".user-name").text("anon (⌐■_■)");$("#account-info").find(".user-email").addClass("hidden");$("#account-info").find(".user-glome-key p").html(localStorage.getItem("glomekey").ellipsis(30));$("#account-info").find(".user-id").html(localStorage.getItem("userid"));$(".sidebar-content").hide();if(!sidebar.isVisible()){sidebar.show()}$("#account-info").show()}}};var login=function(e){e.preventDefault();var email=$("#login-email").val();var password=$("#login-password").val();$.ajax({type:api.createLogin.method,url:api.createLogin.url(),data:{email:email,password:password},success:function(response){localStorage.setItem("token",response.token);$.ajax({method:api.readUser.method,url:api.readUser.url(),headers:{Authorization:"Bearer "+response.token},success:function(data){$("#user-login-dialog").hide();localStorage.setItem("classic","true");localStorage.setItem("username",data.user.name);localStorage.setItem("userid",data.user.id);localStorage.setItem("useremail",data.user.email);void 0;void 0;switchSession("login");alerts.showAlert(13,"success",1500)}})},error:function(response){void 0;alerts.showAlert(24,"danger",2e3);localStorage.removeItem("token")}})};var checkLogin=function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.readUser.method,url:api.readUser.url(),headers:{Authorization:"Bearer "+useToken},success:function(){if(useToken){switchSession("login")}},error:function(){if(useToken){alerts.showAlert(21,"danger",2e3);switchSession("logout");localStorage.clear();sidebar.show($("#user-login-dialog").show().siblings().hide())}}})};var logout=function(){if(!localStorage.token){alerts.showAlert(23,"info",2e3);localStorage.clear()}else{var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.logoutUser.method,url:api.logoutUser.url(),headers:{Authorization:"Bearer "+useToken},success:function(){switchSession("logout");alerts.showAlert(22,"info",2e3);localStorage.clear();if(sidebar.isVisible()){sidebar.hide()}},error:function(){alerts.showAlert(10,"danger",2e3)}})}};var register=function(e){e.preventDefault();var email=$("#register-email").val();var name=$("#register-name").val();var password=$("#register-password").val();$.ajax({type:api.createUser.method,url:api.createUser.url(),data:{email:email,password:password,name:name},success:function(response){localStorage.setItem("token",response.token);$("#create-account-dialog").hide();$.ajax({method:api.readUser.method,url:api.readUser.url(),headers:{Authorization:"Bearer "+response.token},success:function(data){void 0;localStorage.setItem("classic","true");localStorage.setItem("username",data.user.name);localStorage.setItem("userid",data.user.id);localStorage.setItem("useremail",data.user.email);switchSession("login");alerts.showAlert(13,"success",2e3)}})},error:function(response){alerts.showAlert(1,"danger",3500);localStorage.removeItem("token")}})};var glomeGo=function(){void 0;$.ajax({type:api.createSoftAccount.method,url:api.createSoftAccount.url(),dataType:"json",success:function(response){var glomeid=response.user.name,authUser=response.user,token=response.token;if(!glomeid||typeof glomeid==="undefined"){alerts.showAlert(12,"warning",3e3);return}localStorage.setItem("token",token);localStorage.setItem("glomekey",glomeid);localStorage.setItem("userid",response.user.id);$("#user-login-dialog").hide();$.ajax({method:api.readSoftAccount.method,url:api.readSoftAccount.url(glomeid),headers:{Authorization:"Bearer "+token},dataType:"json",success:function(data){void 0;if(!data||typeof data==="undefined"){return}if(typeof authUser!=="undefined"){localStorage.setItem("classic","false");switchSession("login");alerts.showAlert(13,"success",2e3)}}})},error:function(response){void 0;alerts.showAlert(12,"warning",3e3);localStorage.removeItem("token")}})};var deleteAccount=function(e){var classicSessionType=localStorage.getItem("classic");if(classicSessionType==="true"){void 0;e.preventDefault();var useToken=localStorage.getItem("token"),email=localStorage.getItem("useremail"),password=$("#delete-password").val();$.ajax({method:api.removeUser.method,url:api.removeUser.url(),headers:{Authorization:"Bearer "+useToken},data:{email:email,password:password},success:function(){switchSession("logout");alerts.showAlert(17,"success",2e3);localStorage.clear()},error:function(){sidebar.hide();alerts.showAlert(10,"danger",2e3)}})}else{alerts.showAlert(18,"warning",2e3)}};var sendKey=function(e){e.preventDefault();var glomeKey=$("#glome-key").val();$.ajax({type:api.checkGlomeKey.method,url:api.receiveGlomeKey.url(),data:{key:glomeKey},success:function(response){localStorage.setItem("token",response.token);void 0;$("#create-account-dialog").hide();switchSession("login");alerts.showAlert(20,"success",2e3);$.ajax({method:"get",url:"http://api.garbagepla.net/api/authenticate/glome",headers:{Authorization:"Bearer "+response.token},success:function(data){void 0;$("#account-info").find(".user-glome-key p").html(data.user.key);$("#account-info").find(".user-email").addClass("hidden");$("#account-info").find(".created-at").html(data.user.created_at);$("#account-info").find(".updated-at").html(data.user.updated_at);$("#account-info").show()}})},error:function(response){void 0;alerts.showAlert(19,"danger",3e3);localStorage.removeItem("token")}})};return{login:login,checklogin:checkLogin,logout:logout,register:register,glomego:glomeGo,deleteaccount:deleteAccount,sendkey:sendKey}}();$(function(){$(".btn-logout").on("click",session.logout);$(".btn-login").on("click",session.login);$("#registration-form").submit(session.register);$(".btn-glome-go").on("click",session.glomego);$(".btn-glome-key-send").on("click",session.sendkey);$(".btn-delete-account").one("click",session.deleteaccount);if(localStorage.getItem("token")){session.checklogin()}else{return}});
var alerts={texts:["You cannot delete features you did not create","Sorry, something went wrong with the server","The request was not handled properly by the server","Please log in to do that","Something went wrong, HTTP error ","Something went wrong while fetching the data","Failed to remove this feature","Feature deleted successfully","You are not on the players list and can't look at this data","You cannot edit feature created by others","Something went wrong","The editing system isn't currently functional","Failed to login anonymously. Reload the page and try again","You are now logged in","Drawing on mobile is still in development, expect issues","Zoom in closer to create features","Couldn't find your position","Account successfully deleted","You cannot delete soft accounts","Sorry. Key not recognized","Welcome back, anonymous","Session expired. Please log in again","You are logged out","No active session","Failed to log in","Marker saved successfully","Cleaning event saved successfully","Litter saved successfully","Area saved successfully"],showAlert:function(errorCode,errorType,closeDelay){if(!errorType||typeof errorType==="undefined"){var errorType="info"}var errorMessage=this.texts[errorCode];var alertMessage=$('<div class="alert alert-'+errorType+' fade in">').append(errorMessage);$(".alert-container").prepend(alertMessage);if(closeDelay){window.setTimeout(function(){alertMessage.alert("close")},closeDelay)}},showErrorType:function(e){var errortype;if(e.status===200){errortype=this.showAlert(1,"danger",2500)}else if(e.status==="error"){errortype=this.showAlert(2,"danger",2500)}else if(e.status===401||e.status===400){errortype=this.showAlert(3,"danger",2500)}else{errortype=this.showAlert(4+e.status,"danger",2500)}return errortype}};
var map=L.map("map",{zoomControl:false,attributionControl:true});var hash=new L.Hash(map);var baselayer={"Mapbox Outdoors":L.tileLayer("https://api.tiles.mapbox.com/v4/adriennn.9da931dd/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYWRyaWVubm4iLCJhIjoiNWQ5ZTEwYzE0MTY5ZjcxYjIyNmExZDA0MGE2MzI2YWEifQ.WGCZQzbVhF87_Z_Yo1aMIQ",{maxZoom:18,reuseTiles:true,attribution:'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a>, Imagery &copy; <a href="http://mapbox.com">Mapbox</a>'}),"Mapbox Satellite":L.tileLayer("https://api.tiles.mapbox.com/v4/adriennn.nej0l93m/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYWRyaWVubm4iLCJhIjoiNWQ5ZTEwYzE0MTY5ZjcxYjIyNmExZDA0MGE2MzI2YWEifQ.WGCZQzbVhF87_Z_Yo1aMIQ",{maxZoom:18,reuseTiles:true,attribution:'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a>, Imagery &copy; <a href="http://mapbox.com">Mapbox</a>'})};baselayer["Mapbox Outdoors"].addTo(map);var garbageLayerGroup=new L.LayerGroup,areaLayerGroup=new L.FeatureGroup,litterLayerGroup=new L.FeatureGroup,cleaningLayerGroup=new L.LayerGroup,allLayers=new L.LayerGroup([garbageLayerGroup,areaLayerGroup,cleaningLayerGroup,litterLayerGroup]);map.addLayer(garbageLayerGroup,cleaningLayerGroup,litterLayerGroup,areaLayerGroup);var overlayGroups={"Garbage markers":garbageLayerGroup,"Cleaning events":cleaningLayerGroup,"Littered coasts and roads":litterLayerGroup,"Tiles and areas":areaLayerGroup};var zoomcontrol=L.control.zoom({position:"topleft"});var locationcontrol=L.control.locate({position:"topleft"});var scalecontrol=L.control.scale({metric:true,imperial:false});var layerscontrol=L.control.layers(baselayer,overlayGroups,{position:"topleft"});var sidebar=L.control.sidebar("sidebar",{position:"right",closebutton:"true"});var bottombar=L.control.sidebar("bottombar",{position:"bottom",closebutton:"true"});var geocodercontrol=new L.Control.openCageSearch({key:"2bb5bf0d3b9300eacceb225f3cf9cd7d",limit:5,position:"topleft"});map.doubleClickZoom.disable();zoomcontrol.addTo(map);locationcontrol.addTo(map);scalecontrol.addTo(map);layerscontrol.addTo(map);geocodercontrol.addTo(map);sidebar.addTo(map);bottombar.addTo(map);if(window.innerWidth<768){map.removeControl(zoomcontrol)}$(".leaflet-control-layers-toggle").append("<span class='fa fa-2x fa-globe fa-icon-control-centered'></span>");var locating=function(){var coordsinhrf=window.location.href.match(/[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)\/*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)/);var checkHref=function(){if(!coordsinhrf){locationcontrol.start()}};var onLocationFound=function(e){map.setView(e.latlng,18)};var onLocationError=function(e){alerts.showAlert(16,"warning",2e3);if(coordsinhrf){locationcontrol.stop()}else{map.setView([0,0],2)}};map.on("locationerror",onLocationError);map.on("locationfound",onLocationFound)}();
var mapMarker=L.DivIcon.extend({options:{iconSize:[30,30],className:"map-marker",html:'<i class="fa fa-fw"></i>'}});var genericMarker=new mapMarker({className:"map-marker marker-color-gray marker-generic"}),garbageMarker=new mapMarker({className:"map-marker marker-garbage"}),cleaningMarker=new mapMarker({className:"map-marker marker-cleaning"}),dieoffMarker=new mapMarker({className:"map-marker marker-dieoff"}),sewageMarker=new mapMarker({className:"map-marker marker-sewage"}),floatingMarker=new mapMarker({className:"map-marker marker-floating"});
$(document).ready(function(){$("#user-tools").on("click","a",function(e){if($(this).hasClass("dropdown-link")){e.preventDefault();bottombar.hide();sidebar.show();$(this.hash).fadeIn().siblings().hide()}});$("#btn-trashbins").on("click",function(){osmTrashbinLayer=new L.OverPassLayer({query:'(node["amenity"="waste_basket"]({{bbox}});node["amenity"="recycling"]({{bbox}});node["amenity"="waste_disposal"]({{bbox}}););out;'});map.addLayer(osmTrashbinLayer)})});
$(document).ready(function(){$(".sidebar-link").click(function(e){e.preventDefault();$(this.hash).fadeIn().siblings().hide();$("#sidebar").scrollTop=0});$(".menu-backlink").click(function(e){$(".panel-collapse").collapse("hide");$("#sidebar").scrollTop=0;$(this.hash).fadeIn().siblings().hide();e.preventDefault()});$(".btn-cancel").on("click",function(){sidebar.hide();map.removeLayer(marker)})});sidebar.on("hide",function(){$(".sidebar-content").hide();$(".sidebar-container",".sidebar-content").scrollTop=0;$("form").each(function(){this.reset()});$("input").val("");$(".selectpicker").selectpicker("render");$(".tab-default").tab("show");$(".leaflet-draw-edit-edit").removeClass("visible");$(".leaflet-draw-edit-remove").removeClass("visible")});var credits=function(){var content=[{title:"homepage",linkurl:"http://home.garbagepla.net/",text:"Project"},{title:"Let's Encrypt",linkurl:"https://letsencrypt.org/",text:"Secured with"},{title:"Mapbox",linkurl:"https://www.mapbox.com/",text:"Basemaps imagery ©"},{title:"Openstreetmap and contributors",linkurl:"http://www.openstreetmap.org/",text:"Maps and underlying data ©"},{title:"Overpass API",linkurl:"http://www.overpass-api.de/",text:"POIs retrieved using the"},{title:"OpenCage Geocoder",linkurl:"https://geocoder.opencagedata.com/",text:"Address search and geocoding"},{title:"Leaflet",linkurl:"http://leafletjs.com/",text:"Mapping done with "},{title:"FontAwesome",linkurl:"http://fontawesome.io/",text:"Icons by"},{title:"Bootstrap 3",linkurl:"http://getbootstrap.com/",text:"Built with"},{title:"JQuery",linkurl:"https://jquery.com/",text:"Runs on"},{title:"Laravel 5.1",linkurl:"https://laravel.com/",text:"Backed by"},{title:"JavaScript-Templates",linkurl:"https://github.com/blueimp/JavaScript-Templates",text:"Templating with"},{title:"Github",linkurl:"https://github.com/garbageplanet",text:"Source code available on"}];document.getElementById("credits").innerHTML=tmpl("tmpl-credits",content)}();
function pushDataToBottomPanel(e){var featuredata=e.options;if(!featuredata){featuredata=e}String.prototype.insert=function(index,string){if(index>0){return this.substring(0,index)+string+this.substring(index,this.length)}else{return string+this}};document.getElementById("bottombar-content-container").innerHTML=tmpl("tmpl-feature-info",featuredata);sharing.shareThisFeature(featuredata);document.getElementById("social-links").innerHTML=tmpl("tmpl-social-links",sharing.network);bottombar.show($("#feature-info").fadeIn());if(featuredata.image_url){var image_url_insert=featuredata.image_url.insert(26,"b");$("#feature-info").find(".feature-image").attr("src",image_url_insert)}$("#feature-info").find(".btn-edit").on("click",function(){if(localStorage.getItem("token")){editFeature(featuredata)}else{alerts.showAlert(3,"warning",2e3)}});$("#feature-info").find(".btn-cleaned").on("click",function(){cleanedGarbage(featuredata)});$("#feature-info").find(".btn-confirm-garbage").on("click",function(){confirmGarbage(featuredata)});$("#feature-info").find(".btn-attend-cleaning").on("click",function(){attendCleaning(featuredata)});$("#feature-info").find(".btn-participate-game").on("click",function(){participateGame(featuredata)});$(".btn-social").popover({html:true,container:"body",placement:function(pop){if(window.innerWidth<560){return"top"}else{return"right"}},content:function(){return $("#social-links").html()},template:'<div class="popover popover-share" role="tooltip"><div class="popover-content popover-share"></div></div>'});$("#feature-info").find(".btn-delete").on("click",function(e){void 0;e.preventDefault();if(featuredata.feature_type){switch(featuredata.feature_type){case"marker_cleaning":var deletemethod=api.deleteCleaning.method;var deleteurl=api.deleteCleaning.url(featuredata.id);break;case"polyline_litter":var deletemethod=api.deleteLitter.method;var deleteurl=api.deleteLitter.url(featuredata.id);break;case"polygon_area":var deletemethod=api.deleteArea.method;var deleteurl=api.deleteArea.url(featuredata.id);break;case"marker_garbage":var deletemethod=api.deleteTrash.method;var deleteurl=api.deleteTrash.url(featuredata.id);break}var useToken=localStorage.getItem("token")||window.token;if((featuredata.marked_by||featuredata.created_by)==localStorage.getItem("userid")){$.ajax({type:deletemethod,url:deleteurl,headers:{Authorization:"Bearer "+useToken},success:function(response){bottombar.hide();features.loadAllFeatures();alerts.showAlert(7,"success",1500)},error:function(response){void 0;alerts.showAlert(6,"warning",2e3)}})}else{bottombar.hide();alerts.showAlert(0,"danger",2500)}}})}bottombar.on("hide",function(){$(".btn-social").popover("destroy");$(".modal").modal("hide").data("bs.modal",null)});
var features=function(){var garbageArray=[];var cleaningArray=[];var loadGarbageMarkers=function(){garbageArray=[];setTimeout(function(){garbageLayerGroup.clearLayers();var useToken=localStorage.getItem("token")||window.token;$.ajax({type:api.readTrashWithinBounds.method,url:api.readTrashWithinBounds.url(tools.getCurrentBounds()),success:function(data){$(data).each(function(index,obj){garbageArray.push(obj);var latlng=obj.latlng.toString().replace(/,/g,"").split(" ");var marker=new L.Marker(new L.LatLng(latlng[0],latlng[1]),{icon:garbageMarker,id:obj.id,amount:obj.amount,types:obj.types.join(", "),image_url:obj.image_url,latlng:obj.latlng,confirms:obj.confirms,todo:tools.setTodoFullText(obj.todo),tags:obj.tag,note:obj.note,feature_type:"marker_garbage",size:obj.size,embed:obj.embed,created_by:obj.marked_by,created_at:obj.created_at,modified_at:obj.updated_at,cleaned:obj.cleaned,cleaned_by:obj.cleaned_by,cleaned_date:obj.cleaned_date});garbageLayerGroup.addLayer(marker);$(marker._icon).addClass(tools.setMarkerClassColor(obj.amount));map.addLayer(garbageLayerGroup);marker.on("click",function(){featureClick(marker);void 0;var currentZindex=marker._zIndex;marker.setZIndexOffset(currentZindex+1e4)})})},error:function(data){void 0}})},100)};var loadCleaningMarkers=function(){cleaningArray=[];setTimeout(function(){cleaningLayerGroup.clearLayers();var useToken=localStorage.getItem("token")||window.token;$.ajax({type:api.readCleaningWithinBounds.method,url:api.readCleaningWithinBounds.url(tools.getCurrentBounds()),success:function(data){$(data).each(function(index,obj){cleaningArray.push(obj);var latlng=obj.latlng.toString().replace(/,/g,"").split(" ");var marker=new L.Marker(new L.LatLng(latlng[0],latlng[1]),{icon:cleaningMarker,id:obj.id,datetime:obj.datetime,latlng:obj.latlng,feature_type:"marker_cleaning",attends:obj.attends,recurrence:obj.recurrence,created_by:obj.created_by,created_at:obj.created_at,modified_at:obj.updated_at});cleaningLayerGroup.addLayer(marker);map.addLayer(cleaningLayerGroup);$(marker._icon).addClass("marker-color-blue");marker.on("click",function(){featureClick(marker)})})},error:function(data){void 0}})},200)};var loadAreas=function(){setTimeout(function(){areaLayerGroup.clearLayers();var useToken=localStorage.getItem("token")||window.token;$.ajax({type:api.readAreaWithinBounds.method,url:api.readAreaWithinBounds.url(tools.getCurrentBounds()),headers:{Authorization:"Bearer "+useToken},success:function(data){$(data).each(function(index,obj){var latlngs=JSON.parse("["+obj.latlngs+"]");var polygonLayer=new L.Polygon(latlngs,{id:obj.id,title:obj.title,max_players:obj.max_players,curr_players:obj.curr_players,note:obj.note,tags:obj.tag,contact:obj.contact,feature_type:"polygon_area",shape:true,created_by:obj.created_by,created_at:obj.created_at,modified_at:obj.updated_at,color:"#33cccc",weight:5,opacity:.5,smoothFactor:3});areaLayerGroup.addLayer(polygonLayer);map.addLayer(areaLayerGroup);polygonLayer.on("click",function(){featureClick(polygonLayer)})})},error:function(data){void 0}})},400)};var loadLitters=function(){setTimeout(function(){litterLayerGroup.clearLayers();var useToken=localStorage.getItem("token")||window.token;$.ajax({type:api.readLitterWithinBounds.method,url:api.readLitterWithinBounds.url(tools.getCurrentBounds()),headers:{Authorization:"Bearer "+useToken},success:function(data){$(data).each(function(index,obj){var latlngs=JSON.parse("["+obj.latlngs+"]");var polylineLayer=L.polyline(latlngs,{id:obj.id,amount:obj.amount,types:obj.types.join(", "),image_url:obj.image_url,tags:obj.tag,shape:true,feature_type:"polyline_litter",created_by:obj.marked_by,cleaned:obj.cleaned,confirms:obj.confirms,cleaned_by:obj.cleaned_by,created_at:obj.created_at,modified_at:obj.updated_at,physical_length:obj.physical_length,weight:15,opacity:.5,smoothFactor:3,color:tools.setPolylineColor(obj.amount)});litterLayerGroup.addLayer(polylineLayer);map.addLayer(litterLayerGroup);polylineLayer.on("click",function(){featureClick(polylineLayer)})})},error:function(data){void 0}})},300)};var loadAllFeatures=function(){loadGarbageMarkers;loadCleaningMarkers;loadAreas;loadLitters};map.on("dragend zoomend",function(e){if(e.type==="zoomend"){if(e.target.getZoom()>=8){loadGarbageMarkers();loadCleaningMarkers();if(e.target.getZoom()<=16){loadLitters();loadAreas()}}if(e.target.getZoom()<7){areaLayerGroup.clearLayers;litterLayerGroup.clearLayers}}if(e.type==="dragend"){if(e.distance>=window.innerWidth/3){if(e.target.getZoom()>=8){loadGarbageMarkers();loadCleaningMarkers();if(e.target.getZoom()>=8&&e.target.getZoom()<=16){loadLitters();loadAreas()}}}}});return{garbageArray:function(){return garbageArray},cleaningArray:function(){return cleaningArray},loadGarbageMarkers:loadGarbageMarkers,loadCleaningMarkers:loadCleaningMarkers,loadAreas:loadAreas,loadLitters:loadLitters,loadAllFeatures:loadAllFeatures}}();
map.addOneTimeEventListener("zoomend",function(e){if(e.target.getZoom()<10){alerts.showAlert(15,"info",1200)}});L.Map.prototype.panToOffset=function(latlng,offset,options){var x=this.latLngToContainerPoint(latlng).x-offset[0],y=this.latLngToContainerPoint(latlng).y-offset[1],point=this.containerPointToLatLng([x,y]);return this.setView(point,this._zoom,{pan:options})};function getVerticalOffset(){var vOffset=[0,0];if(window.innerWidth>767){vOffset[1]=-$(window).height()/4+20}else{vOffset[1]=-$(window).height()/4}return vOffset}function getHorizontalOffset(){var hOffset=[0,0];hOffset[0]=-$(window).width()/5;return hOffset}function onMapClick(e){if(!sidebar.isVisible()&&!bottombar.isVisible()&&e.target.getZoom()>=12&&!$(".dropdown").hasClass("open")&&!$(".leaflet-control-layers").hasClass("leaflet-control-layers-expanded")&&!$(".leaflet-control-ocd-search").hasClass("leaflet-control-ocd-search-expanded")&&$(".leaflet-control-ocd-search-alternatives").hasClass("leaflet-control-ocd-search-alternatives-minimized")&&!$(".leaflet-compact-attribution-toggle").is(":checked")&&locationcontrol._active!==true){marker=L.marker(e.latlng,{icon:genericMarker,draggable:true});if($(window).width()<=567){if(!$(".leaflet-marker-menu").is(":visible")){marker.setMenu(mobile.featuremenu);map.addLayer(marker).panTo(marker._latlng);marker.openMenu()}}if($(window).width()>567){map.addLayer(marker);map.panToOffset(marker._latlng,getHorizontalOffset());$(".sidebar-content").hide();$("#sidebar").scrollTop=0;sidebar.show($("#create-marker-dialog").show())}$(".marker-latlng").val(marker._latlng.lat+", "+marker._latlng.lng);$("input[type=radio]").on("change",function(){$(marker._icon).removeClass("marker-generic").addClass("marker-garbage");var selectedValue=parseInt($(this).attr("name"),10);$(marker._icon).removeClass(function(index,css){return(css.match(/(^|\s)marker-color-\S+/g)||[]).join(" ")}).addClass(tools.setMarkerClassColor(selectedValue))});$("#event-date-time-picker").on("dp.change",function(e){var eventDateTime=e.date.format("YYYY-MM-DD HH:MM:SS");$("#date-time-value").val(eventDateTime);$(marker._icon).removeClass("marker-color-gray marker-generic").addClass("marker-cleaning marker-color-blue")});marker.on("dragend",function(event){var newPos=event.target.getLatLng();$(".marker-latlng").val(newPos.lat+", "+newPos.lng)});sidebar.on("hide",function(){if(marker){$(marker._icon).removeClass("marker-garbage");$(marker._icon).removeClass("marker-cleaning");$(marker._icon).removeClass(function(index,css){return(css.match(/(^|\s)marker-color-\S+/g)||[]).join(" ")}).addClass("marker-generic");$(marker._icon).addClass("marker-color-gray");if($(window).width()<=567){map.removeLayer(marker)}}});marker.on("click",onLocalMarkerClick)}else{if(locationcontrol._active){locationcontrol.stop()}if($(".leaflet-control-ocd-search").hasClass("leaflet-control-ocd-search-expanded")){$(".leaflet-control-ocd-search").removeClass("leaflet-control-ocd-search-expanded");$(".leaflet-control-ocd-search-alternatives").addClass("leaflet-control-ocd-search-alternatives-minimized");$(".leaflet-control-ocd-search-icon").removeClass("fa-close").addClass("fa-search");return}if(bottombar.isVisible()||sidebar.isVisible()){bottombar.hide();sidebar.hide();return}if($(".dropdown").hasClass("open")){$(".dropdown").removeClass("open");return}if($(".leaflet-control-layers").hasClass("leaflet-control-layers-expanded")){$(".leaflet-control-layers").removeClass("leaflet-control-layers-expanded");return}if($(".leaflet-compact-attribution-toggle").is(":checked")){$(".leaflet-compact-attribution-toggle").prop("checked",false);return}if(e.target.getZoom()<12){alerts.showAlert(15,"info",1200)}}}map.on("click",onMapClick);function onLocalMarkerClick(e){bottombar.hide();marker=this;$(".marker-latlng").val(marker._latlng.lat+", "+marker._latlng.lng);if($(window).width()<=567){map.addLayer(marker).panTo(marker._latlng);return}if($(window).width()>567){map.panToOffset(marker._latlng,getHorizontalOffset());$("#sidebar").scrollTop=0;$(".sidebar-content").hide();sidebar.show($("#create-marker-dialog").fadeIn());marker.on("dragend",function(event){var newPos=event.target.getLatLng();$(".marker-latlng").val(newPos.lat+", "+newPos.lng)})}}
function featureClick(e){featureClick:{if(window.innerWidth<768){if($(".leaflet-marker-menu").is(":visible")){marker.closeMenu();break featureClick}}if(e.options){if(e.options.shape){setTimeout(function(){map.panToOffset(e.getBounds().getCenter(),getVerticalOffset())},100)}else{var latlng=e.options.latlng.toString().replace(/,/g,"").split(" ");map.panToOffset([latlng[0],latlng[1]],getVerticalOffset())}pushDataToBottomPanel(e);if(sidebar.isVisible()){sidebar.hide()}}}}function editFeature(e){var userid=localStorage.getItem("userid");if(userid==e.marked_by){bottombar.hide();alerts.showAlert(11,"warning",3e3);if(e.feature_type==="marker_garbage"){sidebar.show($("#create-garbage-dialog").fadeIn())}if(e.feature_type==="marker_cleaning"){sidebar.show($("#create-cleaning-dialog").fadeIn())}if(e.feature_type==="polyline_litter"){sidebar.show($("#create-litter-dialog").fadeIn())}if(e.feature_type==="polygon_area"){sidebar.show($("#create-area-dialog").fadeIn())}}else{alerts.showAlert(9,"danger",3e3)}}function confirmGarbage(e){if(!localStorage.getItem("token")){alerts.showAlert(3,"info",2e3)}else{var callurl=null;if(e.feature_type==="marker_garbage"){callurl=api.confirmTrash.url(e.id)}else if(e.feature_type==="polyline_litter"){callurl=api.confirmLitter.url(e.id)}setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:"PUT",url:callurl,headers:{Authorization:"Bearer"+useToken},success:function(response){var message=response.data.message;pushDataToBottomPanel(response.data.data);if(message.indexOf("litter")===0){loadLitters()}else{loadGarbageMarkers()}},error:function(err){alerts.showAlert(10,"info",2e3)}})},100)}}function attendCleaning(e){if(!localStorage.getItem("token")){alerts.showAlert(3,"info",2e3)}else{setTimeout(function(){var useToken=localStorage.getItem("token")||window.token,id=e.id;$.ajax({method:"PUT",url:api.attendCleaning.url(id),headers:{Authorization:"Bearer"+useToken},success:function(response){pushDataToBottomPanel(response.data.data);loadCleaningMarkers()},error:function(err){alerts.showAlert(10,"info",2e3)}})},100)}}function participateGame(e){if(!localStorage.getItem("token")){alerts.showAlert(3,"info",2e3)}setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.joinCleaning.method,url:api.readCleaning.url(),headers:{Authorization:"Bearer"+useToken},data:{confirm:data},success:function(data){void 0},error:function(err){void 0;alerts.showAlert(10,"info",2e3)}})},100)}function cleanedGarbage(e){if(!localStorage.getItem("token")){alerts.showAlert(3,"info",2e3)}else{setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.confirmTrash.method,url:api.confirmTrash.url(),headers:{Authorization:"Bearer"+useToken},data:{clean:1},success:function(data){void 0},error:function(err){void 0}})},100)}}
$(function(){$(".image-uploader").fileupload({headers:{Authorization:"Client-ID 6f050e213f46ba9"},type:"POST",url:"https://api.imgur.com/3/upload",dataType:"json",paramName:"image",progressall:function(e,data){var progress=parseInt(data.loaded/data.total*100,10);$(".progress").removeClass("hidden");$(".progress-bar").css("width",progress+"%")},fail:function(err){void 0;$(".progress").addClass("hidden").delay(400)},error:function(err){void 0;$(".progress").addClass("hidden").delay(400)},done:function(e,data){void 0;$(e.target).parent().next().val(data.result.data.link);$(".progress").addClass("hidden").delay(200)}});$(".btn-image-uploader").on("click",function(){if(localStorage.getItem("token")){$(this).next().trigger("click")}else{alerts.showAlert(3,"warning",2e3);return}})});
window.token="eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOjYsImlzcyI6Imh0dHA6XC9cL2FwaS5nYXJiYWdlcGxhLm5ldFwvYXBpXC9hdXRoZW50aWNhdGUiLCJpYXQiOiIxNDQ2OTAxNTcxIiwiZXhwIjoiMTQ0NjkwNTE3MSIsIm5iZiI6IjE0NDY5MDE1NzEiLCJqdGkiOiJhMzljOTg1ZDZmNWNjNmU4MGNlMmQzOWZjODg5NWM1YSJ9.R28VF7VI1S3-PpvaG6cjpyxpygvQCB0JXF5oQ27TxCw";var saving=function(){var saveGarbage=function(e){e.preventDefault();var that=this,garbageType=[],tags=[],garbageTodo,garbageAmount,latlng,image_url,garbageSize=[],garbageEmbed=[],note;$(this).find(".selectpicker.garbage-type-select option:selected").each(function(index,value){garbageType.push($(value).val())});garbageTodo=$(this).find(".selectpicker.garbage-todo-select option:selected").val();$(this).find(".selectpicker.garbage-size-select option:selected").each(function(index,value){garbageSize.push($(value).val())||""});$(this).find(".selectpicker.garbage-embed-select option:selected").each(function(index,value){garbageEmbed.push($(value).val())||""});garbageAmount=$(".btn-group-amount").find(".active > input").attr("name")||"3";note=$(this).find(".garbage-note").val()||"";image_url=$(this).find(".garbage-image-hidden-value").val()||"";tags=$(this).find(".garbage-tags").tagsinput("items")||"";latlng=$(this).find(".garbage-latlng").val();setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.createTrash.method,url:api.createTrash.url(),headers:{Authorization:"Bearer "+useToken},data:{latlng:latlng,amount:garbageAmount,types:garbageType.join(),todo:garbageTodo,image_url:image_url,tag:tags.join(),sizes:garbageSize.join(),embed:garbageEmbed.join(),note:note},success:function(data){void 0;alerts.showAlert(25,"success",1500);sidebar.hide("slow");map.removeLayer(marker);features.loadGarbageMarkers()},error:function(response){alerts.showErrorType(response);sidebar.hide();map.removeLayer(marker)}})},100)};var saveCleaning=function(e){e.preventDefault();var that=this,tags=[],eventRecurrence,dateTime,note,latlng;dateTime=$("#date-time-value").val();tags=$(this).find(".cleaning-tags").tagsinput("items")||"";note=$(this).find(".cleaning-note").val()||"";latlng=$(this).find(".cleaning-latlng").val();eventRecurrence=$(this).find(".selectpicker.cleaning-recurrent-select option:selected").val();setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.createCleaning.method,url:api.createCleaning.url(),headers:{Authorization:"Bearer "+useToken},dataType:"json",data:{latlng:latlng,datetime:dateTime,note:note,recurrence:eventRecurrence,tag:tags.join()},success:function(data){alerts.showAlert(26,"success",2e3);sidebar.hide("slow");features.loadCleaningMarkers();map.removeLayer(marker)},error:function(response){alerts.showErrorType(response);sidebar.hide();map.removeLayer(marker)}})},100)};var saveLitter=function(e){$(".btn-draw").removeClass("disabled");e.preventDefault();var that=this,litterType=[],tags=[],litterAmount,latlngs,image_url,length,note,amount_quantitative;latlngs=$(this).find(".litter-latlngs").val();$(this).find(".selectpicker.litter-type-select option:selected").each(function(index,value){litterType.push($(value).val())});tags=$(this).find(".litter-tags").tagsinput("items")||"";amount_quantitative=$(this).find(".litter-amount-quantitative").val()||"";litterAmount=$(".btn-group-amount-litter").find(".active > input").attr("name")||"3";physical_length=$(this).find(".litter-path-length").val()||"";image_url=$(this).find(".litter-image-hidden-value").val()||"";note=$("input[class=litter-note]").val()||"";setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.createLitter.method,url:api.createLitter.url(),headers:{Authorization:"Bearer "+useToken},dataType:"json",data:{latlngs:latlngs.toString(),amount:litterAmount,types:litterType.join(),image_url:image_url,tag:tags.join()},success:function(data){void 0;alerts.showAlert(26,"success",1500);sidebar.hide("slow");features.loadLitters()},error:function(response){alerts.showErrorType(response);sidebar.hide();map.removeLayer(polylineLayer)}})},200)};var saveArea=function(e){$(".btn-draw").removeClass("disabled");e.preventDefault();var that=this,tags=[],latlngs,note,secret,max_players,title,contact,game;game=$(".tile-game-check input:checked").length||0;void 0;latlngs=$(this).find(".area-latlngs").val();title=$(this).find(".area-title").val();note=$(this).find(".area-note").val()||"";secret=$(this).find(".area-secret").val()||"";contact=$(this).find(".area-contact").val()||"";max_players=$(this).find(".area-players").val()||"";tags=$(this).find(".area-tags").tagsinput("items")||"";function randomString(len){var randomStringValue=" ",charset="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";for(var i=0;i<len;i++){randomStringValue+=charset.charAt(Math.floor(Math.random()*charset.length))}return randomStringValue}if(!title){title=randomString(12);void 0}setTimeout(function(){var useToken=localStorage.getItem("token")||window.token;$.ajax({method:api.createArea.method,url:api.createArea.url(),headers:{Authorization:"Bearer "+useToken},dataType:"json",data:{latlngs:latlngs,note:note,contact:contact,secret:secret,title:title,tag:tags.join(),game:game},success:function(data){void 0;alerts.showAlert(27,"success",1500);sidebar.hide("slow");features.loadAreas()},error:function(response){alerts.showErrorType(response);sidebar.hide();map.removeLayer(polygoneLayer)}})},200)};return{saveGarbage:saveGarbage,saveCleaning:saveCleaning,saveLitter:saveLitter,saveArea:saveArea}}();$(function(){$(".form-garbage").on("submit",saving.saveGarbage);$(".form-cleaning").on("submit",saving.saveCleaning);$(".form-litter").on("submit",saving.saveLitter);$(".form-area").on("submit",saving.saveArea)});
var forms=function(){var garbagetypes=[{"short":"plastic","long":"Plastic items"},{"short":"bags","long":"Plastic bags"},{"short":"foodpacks","long":"Plastic food containers"},{"short":"pet","long":"PET bottles"},{"short":"party","long":"Party leftovers"},{"short":"fastfood","long":"Fastfood garbage"},{"short":"poly","long":"Expanded plastic polymers"},{"short":"butts","long":"Cigarette butts"},{"short":"glassb","long":"Broken glass"},{"short":"glass","long":"Glass"},{"short":"bottles","long":"Glass bottles"},{"short":"metal","long":"Metal"},{"short":"tin","long":"Tin cans"},{"short":"alu","long":"Aluminium cans"},{"short":"wood","long":"Plywood and treated or painted wood"},{"short":"chemicals","long":"Chemicals"},{"short":"household","long":"Household garbage"},{"short":"clothes","long":"Shoes and clothes"},{"short":"fabric","long":"Carpets and fabrics"},{"short":"matress","long":"Matresses"},{"short":"tarp","long":"Tarps and other large covers"},{"short":"electronic","long":"Electronics"},{"short":"electric","long":"Electric appliances"},{"short":"batt","long":"Batteries"},{"short":"industrial","long":"Industrial wastes"},{"short":"construction","long":"Construction wastes"},{"short":"gas","long":"Gasoline and petroleum oil"},{"short":"crude","long":"Crude oil"},{"short":"vehicle","long":"Large vehicle"},{"short":"bicycle","long":"Bicycles"},{"short":"motorcyle","long":"Motorcycles"},{"short":"tyres","long":"Tyres"},{"short":"engine","long":"Engine parts"},{"short":"parts","long":"Vehicles parts"},{"short":"fishing","long":"Fishing gears"},{"short":"commercial","long":"Commercial fishing equipment"},{"short":"net","long":"Fishing net"},{"short":"lines","long":"Fishing line"},{"short":"boat","long":"Small boat"},{"short":"vessel","long":"Large boat or wreck"},{"short":"boating","long":"Boating equipment"},{"short":"buoy","long":"Buoys and floats"},{"short":"maritime","long":"Maritime equipment"},{"short":"sew","long":"Sewage"},{"short":"dogs","long":"Dog poop bags"}];document.getElementById("garbage-select").innerHTML=tmpl("tmpl-form-garbage-type",garbagetypes);document.getElementById("litter-select").innerHTML=tmpl("tmpl-form-garbage-type",garbagetypes);$(".selectpicker").selectpicker({style:"btn-lg btn-default text-center",size:6});$("#event-date-time-picker").datetimepicker({minDate:new Date(2016,4,1),showClose:true,ignoreReadonly:true,focusOnShow:false,toolbarPlacement:"top"});$(".feature-tags").tagsinput({maxTags:3,confirmKeys:[32,39],maxChars:16,trimValue:true});$(".form-feature").bind("keypress",function(e){if(e.keyCode===13){$(".btn-save").attr("type");e.preventDefault()}})}();
$("#modal-list-garbage").on("click",function(){$("body").append('<div id="modal-data" class="modal" role="dialog"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close pull-right" data-dismiss="modal"><span class="fa fa-fw fa-times"></span></button><h4 class="modal-title">View and download data from garbagepla.net</h4></div><div class="modal-body"><table id="modal-garbage-table" class="table-striped table-condensed"><thead><tr><th>Feature id</th><th>Coordinates</th><th>Amount</th><th>Garbage types</th></tr></thead><tbody id="modal-garbage-table-body"></tbody></table></div><div class="modal-footer"><a class="disabled pull-left" href="#">Get the full data</a><form><button type="button" data-dismiss="modal" class="btn btn-danger pull-right ">Close</button><button id="modal-data-load-more" type="button" class="btn btn-default pull-right ">Zoom out and load more</button><a id="modal-garbage-download" href="" target="_blank" data-download="data.txt" type="button" class="btn btn-default pull-right">Download</a></form></div></div></div></div>');document.getElementById("modal-garbage-table-body").innerHTML=tmpl("tmpl-modal-garbage",features.garbageArray());$("#modal-data").modal("show");$("#modal-garbage-table").DataTable();$("#modal-data-load-more").on("click",function(){map.setZoom(map.getZoom()-1);$(".modal-data-row").empty();document.getElementById("modal-garbage-table-body").innerHTML=tmpl("tmpl-modal-garbage",features.garbageArray())});$("#modal-garbage-download").on("click",function(){var stringdata="text/json;charset=utf-8,"+JSON.stringify(features.garbageArray());this.setAttribute("href","data:text/plain;charset=utf-8,"+encodeURIComponent(stringdata))})});$("#modal-list-cleaning").on("click",function(){$("body").append('<div id="modal-calendar" class="modal" role="dialog"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close pull-right" data-dismiss="modal"><span class="fa fa-fw fa-times"></span></button><h4 class="modal-title">Cleaning events calendar</h4></div><div class="modal-body"><table id="modal-calendar-table" class="table-striped table-condensed"><thead><tr><th>Event id</th><th>Date</th><th>Coordinates</th><th>Address</th></tr></thead><tbody id="modal-calendar-table-body"></tbody></table></div><div class="modal-footer"><form><button type="button" data-dismiss="modal" class="btn btn-danger pull-right ">Close</button><button id="modal-calendar-load-more" type="button" class="btn btn-default pull-right ">Zoom out and load more</button><a id="modal-calendar-download" href="" target="_blank" data-download="data.txt" type="button" class="btn btn-default pull-right">Download</a></form></div></div></div></div>');document.getElementById("modal-calendar-table-body").innerHTML=tmpl("tmpl-modal-cleaning",features.cleaningArray());$("#modal-calendar").modal("show");$("#modal-calendar-table").DataTable();$("#modal-calendar-load-more").on("click",function(){map.setZoom(map.getZoom()-1);$(".modal-calendar-row").empty();document.getElementById("modal-calendar-table-body").innerHTML=tmpl("tmpl-modal-cleaning",features.cleaningArray())});$("#modal-calendar-download").on("click",function(){var stringdata="text/json;charset=utf-8,"+JSON.stringify(features.cleaningMarkers());this.setAttribute("href","data:text/plain;charset=utf-8,"+encodeURIComponent(stringdata))})});
