// TODO make the modals with a single function
// TODO make the function and event listener separate
$('#modal-list-garbage').on('click', function () {
    ...
    document.getElementById('modal-garbage-table-body').innerHTML = tmpl('tmpl-modal-garbage', features.garbageArray());
    ...
    $('#modal-garbage-download').on('click', function () {
        var stringdata = "text/json;charset=utf-8," + JSON.stringify(features.garbageArray());
...
$('#modal-list-cleaning').on('click', function () {
    ...
    document.getElementById('modal-calendar-table-body').innerHTML = tmpl('tmpl-modal-cleaning', features.cleaningArray());
    ...
    $('#modal-calendar-download').on('click', function () {
        var stringdata = "text/json;charset=utf-8," + JSON.stringify(features.cleaningMarkers());
        this.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(stringdata));            
    });
});